...
export type Props<Route extends BaseRoute> = {
   ...
   * ```
   *
   * `LeftNavigation.Bar` is a controlled component, which means the `index` needs to be updated via the `onTabPress` callback.
   */
...
/**
 * A navigation bar which can easily be integrated with [React Navigation's Left Tabs Navigator](https://reactnavigation.org/docs/bottom-tab-navigator/).
 *
 * ## Usage
 * ```js
 ...
 *
 * import { createLeftTabNavigator } from '@react-navigation/bottom-tabs';
 * import { Text, LeftNavigation } from 'react-native-paper';
 ...
 * ```
 */
const LeftNavigationBar = <Route extends BaseRoute>({
  navigationState,
  renderIcon,
  renderLabel,
  ...
  if (shifting && navigationState.routes.length < 2) {
    shifting = false
    console.warn(
      'LeftNavigation.Bar needs at least 2 tabs to run shifting animation'
    )
  }
...
LeftNavigationBar.displayName = 'LeftNavigation.Bar'
