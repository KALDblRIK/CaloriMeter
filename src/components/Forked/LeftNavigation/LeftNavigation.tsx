...
import LeftNavigationRouteScreen from './LeftNavigationRouteScreen'
...
export type Props<Route extends BaseRoute> = {
   ...
   * ```
   *
   * `LeftNavigation` is a controlled component, which means the `index` needs to be updated via the `onIndexChange` callback.
   */
   ...
   *
   * ```js
   * renderScene = LeftNavigation.SceneMap({
   *   music: MusicRoute,
   *   albums: AlbumsRoute,
   * });
   * ```
...
/**
 * LeftNavigation provides quick navigation between top-level views of an app with a bottom navigation bar.
 * It is primarily designed for use on mobile. If you want to use the navigation bar only see [`LeftNavigation.Bar`](LeftNavigationBar).
 *
 * By default LeftNavigation uses primary color as a background, in dark theme with `adaptive` mode it will use surface colour instead.
 * See [Dark Theme](https://callstack.github.io/react-native-paper/docs/guides/theming#dark-theme) for more information.
 *
 * ## Usage
 * ```js
 * import * as React from 'react';
 * import { LeftNavigation, Text } from 'react-native-paper';
 *
 ...
 *   ]);
 *
 *   const renderScene = LeftNavigation.SceneMap({
 *     music: MusicRoute,
 *     albums: AlbumsRoute,
 ...
 * ```
 */
const LeftNavigation = <Route extends BaseRoute>({
  navigationState,
  renderScene,
  renderIcon,
  renderLabel,
  ...
  if (shifting && navigationState.routes.length < 2) {
    shifting = false
    console.warn(
      'LeftNavigation needs at least 2 tabs to run shifting animation'
    )
  }
 ...
 */
LeftNavigation.SceneMap = <Route extends BaseRoute>(scenes: {
...
// @component ./LeftNavigationBar.tsx
LeftNavigation.Bar = LeftNavigationBar
...
function useAnimatedValueArray(arg0: (1 | -1 | 0)[]) {
